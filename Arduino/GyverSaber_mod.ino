// Это модификация оригинального проекта AlexGyver - Световой меч.
// https://community.alexgyver.ru/threads/svetovoj-mech.33/
/*
...
// ---------------------------- НАСТРОЙКИ -------------------------------
#define NUM_LEDS 34         // число МИКРОСХЕМ на ленте
...
#define timeVibr 100        // дополнительное время на вибрацию, иначе очень быстро и не заметно
...
#define BTN 4               // пин кнопки // - ИСПРАВИЛ!
#define VOLT_PIN A6         // пин вольтметра
#define BTN_LED 3           // светодиод кнопки // - ИСПРАВИЛ!
#define vMOTOR 7            // вибромотор // - ИСПРАВИЛ!
...
// создание объектов
CRGB leds[NUM_LEDS];
#define SD_ChipSelectPin 8  // - ИСПРАВИЛ!
TMRpcm tmrpcm;
MPU6050 accelgyro;
// -------------------------- БИБЛИОТЕКИ ---------------------------
...
byte btn_counter;
unsigned long btn_timer, blink_timer, swing_timer, swing_timeout, battery_timer, bzzTimer, ledTimer;
byte nowNumber;
...
boolean eeprom_flag, swing_flag, swing_allow, strike_flag, HUMmode = false;
float voltage;
int blinkOffset;
byte znacLed = 250; // стартовое знвчение для светодиода/индикации // - ИСПРАВИЛ!
boolean ponZnach = true; // направления для мигания индикации // - ИСПРАИЛ!
...
void setup() 
{
  ...
  {  // если был хоть один запуск прошивки
  ...
  {   // отобразить заряд аккумулятора как длину клинка
...
void btnTick() 
{
  ...
  if ((millis() - btn_timer > BTN_TIMEOUT) && (btn_counter != 0)) {
    if (ls_state) {
      if (btn_counter == 1) {               // если число нажатий равно 3
...
void on_off_sound() 
{                // блок вкл/выкл меча со звуками
  if (ls_chg_state) 
  {                // если есть запрос на изменение состояния меча
    if (!ls_state) 
    {                 // если меч выключен
    ...
    if (!ls_state)
    {
      ...
      {
        //Serial.println("znacLed = " + String(znacLed));
        ledTimer = millis(); 
        if (ponZnach)
        {
          if(--znacLed < 10) ponZnach = false;
        } else {
          if(++znacLed > 250) ponZnach = true;
        }
        analogWrite(BTN_LED,znacLed);
      }
    }
  }

  if (((millis() - humTimer) > 9000) && bzzz_flag && HUMmode) 
  {   // если настало время трещать и разрешено трещать
    tmrpcm.play("HUM.wav");
